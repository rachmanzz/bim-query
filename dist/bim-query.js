...
var addUnderscore = function addUnderscore(num) {
  if (bim.isNumber(num)) {
    var result = '';
    ...
    if (/^start_with$|^end_with$|^any_position$|^have_at_[0-9]+$|^start_min_[0-9]+$|^start_with_end$/.test(position)) {
      ...
      if (bim.iString(value)) {
        if (/^have_at_[0-9]+$/.test(position)) {
          var gm = position.match(/^have_at_([0-9]+)$/);
          var num = gm[1];
          var under = addUnderscore(parseInt(num));
          val = under + value;
          pos = 'start_with';
        } else if (/^start_min_[0-9]+$/.test(position)) {
          var _gm = position.match(/^start_min_([0-9]+)$/);

          var _num = _gm[1];

          var _under = addUnderscore(parseInt(_num));
      ...
      if (bim.isNotUndef(this.query.wherelike)) {
        if (bim.isArray(this.query.wherelike)) this.query.wherelike.push(key + '|' + val + '|' + pos);else {
          var arr = [];
